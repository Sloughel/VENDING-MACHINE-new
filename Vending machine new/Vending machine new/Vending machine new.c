...
begin: 
	...
	while (i < 5)
	{
		...
		if(comm_list[i].channel_code>5)
		{
			printf("通道仅有1，2，3.，4，5共五个通道\n");
			goto begin;
		}
...
int pay()
{
	...
	for (i = 0; i <= u; i++)
	{
		...
		pai_money += user_comm_list[i].commodity_num * user_comm_list[i].commodity_price;
	}
	printf("您应付%d元\n", pai_money);
	...
	while (change_all<pai_money)
	{
		...
		printf("change为%d",change);
		
		change_all += change;
		change_for = pai_money - change_all;
		printf("您刚才投入了%d\n", change);
		printf("您已投入%d￥,还应投入%d￥,请继续投币\n ",change_all,change_for);
	}
